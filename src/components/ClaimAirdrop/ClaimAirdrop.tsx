...
class ClaimAirdrop extends Component<Props> {
  ...
  public init = async () => {
    ...
    let balanceData=[];
    let name = ['DBIT BALANCE,DIBT Ⓘ',"DBGT BALANCE,DBGT Ⓘ","LOCKED DIBT,DIBT Ⓘ","LOCKED DBGT,DBGT Ⓘ"];
  ...
  public Balance = () => {
      ...
      return this.state.balanceData.map(({num, type, unit}, i)=>(
        <Col className={styles.item} span={12} key={i}>
          <h3 className={styles.item_title}>{type}</h3>
          <p className={styles.amount}>{num} {unit}</p>
        </Col>
      ))
  }
  ...
  public WalletInfo = (data:any,type:any = false)=>{
    ...
    return (
      <Row className={styles.climAirdrop}>
        <Col span={type&&5 || 5}>{Object.keys(data)[0]} {data.index}</Col>
        <Col span={type&&10 || 14}>{Object.keys(data)[1]} <span className={styles.address}>{data.address}</span></Col>
        <Col span={type&&9 || 5} style={{textAlign:'right'}}>{Object.keys(data)[2]} <span className={styles.amount2}>{data.amount}</span></Col>
      </Row>
    )
  }
  ...
  public handleLuckyOnes = ()=>{
    let curr = this.state.dataSource.filter(({address}, i)=>address == this.props.currAddress);
    if (curr.length){
        ...
        <div className={styles.wrap}>
          ...
          <Row>
            <Col xs={{
              ...
              <Button title="Download airdrop.csv" onClick={()=>{
                window.location.href = window.location.origin + "/airdrop_list.csv"
              }} style={{width:"100%"}} ghost type="default" size="large">CSV</Button>
            </Col>
          </Row>
          <Row>
            <Col  lg={{
