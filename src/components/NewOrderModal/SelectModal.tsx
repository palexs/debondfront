...
import {Button, Checkbox, Col, Collapse, Input, Modal, ModalProps, Row, Slider, Table, Tabs} from 'antd';
...
class SelectModal extends Component<Props> {
  ...
  public generateTemp = (key:number, data:any)=>{
    ...
    if (data){
              ...
              <Row className={styles.panelRow}>
                ...
                <Col span={3}>
                  <div onClick={(e) => {e.stopPropagation(); e.preventDefault(); }}>
                    <Slider value={(parseInt(data.balances))}
                ...
                <Col span={3}>
                  <p className={styles.label}>
                    ...
                    /<span className={styles.labelGrow}>100</span></p>
                </Col>
                <Col span={2}>
                  <span style={{color: "#fff"}}>
              ...
              <div style={{width: "100%"}}>
                  {
                    data.list.map((item: any, index: number) => (
                        <Row key={item.id}>
                          ...
                          <Col span={3}>
                            <Slider value={(parseInt(item.balances))}
                                    className={styles.percentSlider}
                                    tipFormatter={null}
                                    onChange={ (val: number) => this.setItemValue(val, index, key) }/>
                          </Col>
                          <Col span={3}>
                            <p className={styles.label}><span>{ item.balances }</span>/<span>100</span></p>
                          </Col>
                          <Col span={2}>
      ...
      if(currKey == "2"){
        const list = data.list.map((it:any,i:number)=><Col className="item" key={i} span={12}><h4 className="">{it.name}</h4><p>{it.num}</p></Col>)
        return (<Collapse.Panel header={
            <>
              <Row>
    ...
    return (
      <Modal className={`my-modal selectModal ${styles.selectModal_wrap}`} visible={this.props.visible} title={null} footer={null} width="50%">
        ...
        <h1 style={{textAlign: "center", color: "#fff"}}>{this.props.title} <Close
          onClick={this.props.onCancel} className={`close_icon ${styles.close_icon}`}/>
        </h1>
        ...
        </Row>
        {
            this.props.type !== "order" && <Row className="createOrder">
              <Col span={24}>
                <Button size="large" ghost icon={<PlusOutlined />} onClick={this.props.onNewOrder}>Create New Order</Button><span style={{fontSize:30}}>Ⓘ</span>
              </Col>
            </Row>
        }
      </Modal>
