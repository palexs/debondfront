...
import {Button, Col, Collapse, Input, Modal, ModalProps, notification, Row, Slider, Table, Tabs} from 'antd';
...
class SelectModal extends Component<Props> {
  ...
  getDurationString = (duration: number): string => {
    ...
    let minuteTime = undefined;
    let secondTime = undefined;
    let hourTime = undefined;
    let results = '';
    if (seconds > 0) {
      // minutes
  ...
  getERC20LoanAuction = async (curKey?: number) => {
    ...
    let result = await loan.getDEXInfo(key);
    const Bank = new Contract(address,abiTest,this.props.provider);
  ...
  getWalletERC20LoanAuction = async (curKey?: number) => {
    ...
    let result = await loan.getWalletInfo(key);
    const Bank = new Contract(address,abiTest,this.props.provider);
    const tableTit = await Bank.getBondsDexColumns();
    const content = await Bank.getBondsDexData();
    const nftData = await Bank.getBondsDexNFTData();
  ...
  public OtherSymbols = (data: any) => {
    return data.map((it:string, i: number)=>(<Col key={i} span={it=="PROGRESS"&&8||4}><h3 style={{color: "#fff"}}>{it}</h3></Col>))
  }
...
/*    if (this.props.type === 'wallet') {
    ...
    if ( data ) {
      if (currKey == "1"){
        return (<Collapse.Panel header={
            <>
              ...
              <Button onClick={(e)=>{ this.handleTakeOrder(e, data, TAKE_OVER_BUTTON(this.props.type))}}
                      type="default"
            ...
            <Table
              className="tables" dataSource={data.list}
            >
      ...
      if(currKey == "2"){
        const list = data.list.map((it:any,i:number) =>
            <Col className="item" key={i} span={12}><h4 className="">{it.name}</h4><p>{it.num}</p></Col>)
        return (<Collapse.Panel header={
            <>
              <Row>
                <Col span={4}><h3 style={{color: "#fff"}}>{data.symbol}</h3></Col>
              ...
              <Button onClick={(e)=>{this.handleTakeOrder(e,data,this.props.type === "dex" && "Take Order" || "Repay")}}
                      type="default"
                      className="take_order"
                      ghost >
      ...
      if (currKey == "3"){
        const list = data.list.map((it:any,i:number)=><Col className="item" key={Math.random()} span={it.span}><h4 className="">{it.name}</h4><p>{it.num}</p></Col>)
        return (<Collapse.Panel header={
            <>
              <Row>
                <Col span={4}><h3 style={{color: "#fff"}}>{data.symbol}</h3></Col>
            ...
            <Row key={Math.random()} className="nft">
              <Col span={12} className="author">
                <div className="content">
                  <p className="title"> CryptoPunk 42E32b  <IconFont style={{position:"absolute",right:0}} type="icon-lianjie" /> </p>
                </div>
              </Col>
              <Row className="rightData">
                ...
                <Col key={Math.random()} className="item last" span={12}>
                  <h4>SHAREⒾ</h4>
                  <div>
                    <p className="num"><span>{this.state.inputValue}</span>/<span>100</span></p>
                    <Slider key={Math.random()} tooltipVisible={false} onChange={this.handleSlider}
  ...
  render() {
    const {content,erc20LoanData,tableTit,erc20LoanTit,nftData,nftTit} = this.state
    return (
      <Modal className={`my-modal selectModal ${styles.selectModal_wrap}`}
             visible={this.props.visible}
             title={null} footer={null} width="50%">

        <h1 style={{textAlign: "center", color: "#fff"}}>{this.props.title} <Close
          onClick={this.props.onCancel} className={`close_icon ${styles.close_icon}`}/> </h1>
        <Tabs defaultActiveKey={this.state.currKey} centered onChange={this.handleTabsChange}>
          <Tabs.TabPane key={1} tab="ERC-20">
            <h2 className={`title `}
              ...
              {this.props.titleSub}
              <Refresh spinning={this.state.spinning}
          ...
          <Tabs.TabPane key={3} tab="NFT">
            <h2 className={`title ${commonStyles.vCenter}`}
                style={{color: "#fff"}}>
              {this.props.titleSub}
              <Refresh spinning={this.state.spinning}
                       style={{cursor: 'pointer', marginLeft: '5px'}}
        ...
        <Row className="control">
          <Col span={5}>
            <Input className={styles.format_height}
          ...
          <Col span={9}>
            <NavLink to="/bank?airdrop=1">
              <Button className={styles.format_height}
                      size="large"
                      type="primary"
              >Balance 234,234,234,234 dbit</Button>
            </NavLink>
          </Col>
        </Row>
        {
            this.props.type !== "order" && <Row className="createOrder">
              <Col span={24}>
                <Button size="large" ghost icon={<PlusOutlined />} onClick={this.onCreateClick}>Create New Order</Button><span style={{fontSize:30}}>Ⓘ</span>
              </Col>
            </Row>
        }
      </Modal>
