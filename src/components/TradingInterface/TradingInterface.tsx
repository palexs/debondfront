...
import {Button, Col, Input, notification, Row, Slider, Space, Tabs} from "antd";
...
/*
  ...
  public renderCurr = () => {
    return this.state.inputData.map(({eta, name, progress}, i) => {
      if (this.props.type !== "bonds") {
        ...
        >
          <span className={TradingStyles.name}><IconFont type="icon-lianjie-copy"/> {name}</span>
          <span className={TradingStyles.name}>ETA: {eta}</span>
          <span className={TradingStyles.progress}>PROGRESS : <span
            style={{color: "#fff"}}>{make_bar(progress, bar_styles[8], 20, 20).str}</span></span>
        </p>
      } else {
        return <p
          key={i}
  ...
  public handleView = async () => {
    ...
    this.viewTimer = setInterval(() => {
      let d = (Date.now() - this.currentTimestamp) / 1000;
      if (d > 30 * 60) {
        this.setState({
          depositStatus: true,
          outputStatus: false,
  ...
  public updateRepayments = () => {
    this.setState({
      leastRepayment: BigNumber.from(this.state.min).mul(this.state.inTrestReateVal).div(100).add(this.state.min).toNumber(),
      mostRepayment: BigNumber.from(this.state.max).mul(this.state.inTrestReateVal).div(100).add(this.state.max).toNumber()
    })
  }
    ...
    return (
      <>
        ...
        <Row key={192} className={TradingStyles.min_wrap}>
          ...
          <Col className={TradingStyles.amount} span={8} push={7}>
            <Input type="number" autoComplete={'off'} placeholder="LOAN AMOUNT" className={TradingStyles.inputAmount}
                   onChange={(e) => {
                     this.handleValue(e)
                   }} name="min" value={this.state.min}/>
          </Col>
          <Col className={TradingStyles.lastName} span={4} push={7}>
        ...
        <Button type="default"
          ...
          BORROW <span className={TradingStyles.borrow_flag}>Ⓘ</span>
        </Button>
      </>
    )
  }
      ...
      <>
        <Row key={1000000} className={TradingStyles.slider_wrap}>
          <Col span={24}>
            ...
            <Slider value={typeof this.state.stepDueDate === 'number' ? this.state.stepDueDate : 0}
                    style={{width: "50%", margin: '20px auto'}}
                    onChange={this.handleDueDate}/>
            <div className={TradingStyles.infos}>
              <span
                className={`${TradingStyles.money} ${TradingStyles.block}`}>(365 DAYS {this.state.currentDateFormat})</span>
              <Input name="dueDate" key="deuDate" size="small" suffix={"DAYS"} defaultValue={this.state.dueDate}
                     value={this.state.dueDate} onChange={(e) => {
        ...
        <Row key={23000000} className={TradingStyles.slider_wrap}>
          <Col span={24}>
            ...
            <Slider value={this.state.inTrestReateVal}
                    min={0}
                    max={200}
                    style={{width: "50%", margin: '20px auto'}} tipFormatter={null}
                    onChange={this.handleInTrestReateVal}/>
            <div className={TradingStyles.infos}>
              <span>(min:{this.state.leastRepayment} max:{this.state.mostRepayment} )</span>
              <span
                className={`${TradingStyles.proportion} ${TradingStyles.block}`}>{this.state.inTrestReateVal / 2}%</span>
  ...
  public getBankButton = () => {
    ...
    return <Button disabled={this.state.depositStatus}
                   className={`${TradingStyles.btn} ${TradingStyles.deposit} ${this.state.depositStatus && TradingStyles.disabled || ""}`}
    ...
    return (
      <div className={`${TradingStyles.content} buyDbitBond_wrap`} style={{position: 'relative'}}>
        ...
        <div className={TradingStyles.input}>
          ...
          <Slider value={typeof this.props.stepSize === 'number' ? this.props.stepSize : 0}
                  min={0}
                  max={10000}
                  style={{width: "50%", margin: '20px auto'}} tipFormatter={null}
