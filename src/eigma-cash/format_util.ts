...
export async function getChainId(address: string): Promise<{ chainId: number, balance: string, address: string}> {
...
export async function handleBalance(): Promise<{ chainId: number, balance: string, address: string}> {
  const address = await windowNew.ethereum.request({ method: 'eth_requestAccounts' });
  return getChainId(address[0]);
}
function repeat(s: number, i: number): string {
  let r = '';
...
export function make_bar(p: number, bar_style: any, min_size: number, max_size: number): {str?: string, delta: number} {
