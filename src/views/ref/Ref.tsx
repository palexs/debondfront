...
export default class Ref extends Component<Props> {
  ...
  public generateTemp1 = ()=>{
    ...
    return arr.map((it,i)=>(<Col span={8} >
      <h4>{it.name}</h4>
      <p>{it.num} {it.unit}</p>
    </Col>))
  }
  ...
  public generateTemp2=()=>{
    ...
    return this.state.refData.map(({name, num, unit}, i)=>(<Col span={8} >
      <dl>
        <dt>{name} :</dt>
        <dd>{num} {unit}</dd>
      </dl>
  ...
  render() {
    return (
      <div className='ref_wrap'>
        ...
        <Card className='ref_card' title={<>PROPOSALS <RedoOutlined style={{cursor:"pointer"}}  /> </>}>
          {/**
