...
export class ModuleOne extends React.Component<IMyComponentProps, IMyComponentState> {
  ...
  public order = async () => {
    ...
    const address = this.orderAddress ? this.orderAddress : this.currentAddress;
    const ce = await this.contracts.dex.bid(address, this.index);
  };
      ...
      <div>
        <Modal title="Deposit for SASH" visible={this.props.disView} footer={null} onOk={this.props.openView} onCancel={this.props.closeView}>
          <div style={{ marginLeft: ' 9%' }}>
            address:
            {' '}
            <Input type="text" placeholder="(Address of inputs)" style={{ width: '300px', marginBottom: '10px', height: '35px' }} className={styles.amountNmuber} onBlur={this.setAddress} />
          </div>
          <div className={styles.disButton}>

            <button className={styles.buttona} onClick={this.order}>Take Order</button>
          </div>
