...
import {
  Button, Input, Slider, Row, Col, notification,
} from 'antd';
      ...
      <div className={styles.dueDateSlider} key={10000000000000323244}>
        ...
        >
          ...
          <div className={buySashBond.infos} style={{ width: '100%' }}>
            <span className={`${buySashBond.money} ${buySashBond.block}`}>
              (max:
      ...
      <div className={styles.dueDateSlider} key={10000345345000000000324}>
        ...
        >
          ...
          <div className={buySashBond.infos} style={{ width: '100%' }}>
            ...
            <span className={`${buySashBond.proportion} ${buySashBond.block}`}>
              { Math.floor(this.state.dueDate / 3.65) }
              %
            </span>
            <Input
              size="small"
              className="input_erc20"
              suffix="DAYS"
              value={this.state.dueDate}
              onChange={(e) => {
      ...
      <div className={styles.dueDateSlider} key={100003453435435}>
        ...
        >
          ...
          <div className={buySashBond.infos} style={{ width: '100%' }}>
            <span className={`${buySashBond.money} ${buySashBond.block}`}>
              (30 DAYS
              {this.state.currentDurationFormat}
              )
            </span>
            <span className={`${buySashBond.proportion} ${buySashBond.block}`}>
              { Math.floor(this.state.duration / 0.3) }
              %
            </span>
            <Input
              key={120}
        ...
        <div className={styles.createModalContent}>
          <div className={styles.inputWrapper}>
            <div className={styles.inputItem}>
              <h3>MAX</h3>
              <Input
                className={`${styles.CreateAmountInput} ${commonStyles.default_input}`}
            ...
            <div className={styles.inputItem}>
              <h3>MIN</h3>
              <Input
                className={`${commonStyles.default_input} ${styles.CreateAmountInput}`}
                style={{ width: 150, color: '#fff' }}
                size="large"
          ...
          <Row>
            <Col span={11}>
              <Button
                className={`${commonStyles.default_button} ${styles.CreateButton}`}
                size="large"
                type="primary"
            ...
            <Col span={11} push={2}>
              <Button
                className={`${commonStyles.default_button} ${styles.CreateButton}`}
                style={{ opacity: this.state.saveStatus && 0.5 || 1 }}
